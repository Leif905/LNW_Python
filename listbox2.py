...
from alterentry import alterentry
...
def selected_item():
    ...
    for _ in my_listbox.curselection():
        alterentry((index_list[my_listbox.curselection()[0]])[0])
        
        


new_entry_btn= Button(root, text="Neuer Eintrag", command=entrymask).pack(side=tkinter.LEFT)
alter_entry_btn= Button(root, text="Eintrag bearbeiten", command=selected_item).pack(side=tkinter.LEFT)
